...
class SugarBean
{
    ...
    */
     ...
     * @var array
     */
    protected static $default_acls = [];
    /**
     * Default visibility classes, for dynamic ACL/customizations
     * @var array
     */
    protected static $default_visibility = [];
    ...
    /**
     * Setting this to true allows sugar actions batching
     *
     * @var bool
     */
     ...
     * @var array
     */
    public $rel_fields_before_value = [];
     ...
     * @var array
     */
    protected static $recursivelyResavedLinks = [];
    ...
    private $auditedStateQueue = [];

    /**
     * log of used email subjects for email duplicate prevention
     *
    ...
    public function __construct()
    {
        ...
        if (false == $this->disable_vardefs && (empty(self::$loadedDefs[$this->object_name]) || !empty($GLOBALS['reload_vardefs']))) {
            ...
            VardefManager::loadVardef($this->getModuleName(), $this->object_name, $refresh, ['bean' => $this]);
            ...
            $this->field_defs = self::$loadedDefs[$this->object_name]['field_defs'] ?? null;
        ...
        if (!$this->disable_row_level_security && !empty($current_user->id) && !isset($this->disable_team_security)
            && !SugarACL::checkAccess($this->module_dir, 'team_security', ['bean' => $this])) {
    ...
    public function loadVisibility()
    {
        if (empty($this->visibility)) {
            $data = $GLOBALS['dictionary'][$this->object_name]['visibility'] ?? [];
     ...
     * Internal function, do not override.
     */
    public function getActivityEnabledFieldDefinitions($excludeType = ['datetime'])
    {
        if (!isset($this->activity_enabled_fields)) {
            $this->activity_enabled_fields = [];
            foreach ($this->field_defs as $field => $properties) {
                $field_type = '';
                if (isset($properties['type'])) {
                    ...
                    } elseif (isset($properties['data_type'])) {
     ...
     */
    public function getFieldDefinitions(?string $property = null, array $filter = []): array
    {
    ...
    public function unPopulateDefaultValues()
    {
        ...
        foreach ($this->field_defs as $field => $value) {
            ...
            if (!empty($this->$field) && !empty($value['display_default']) && in_array($value['type'], ['date', 'datetime', 'datetimecombo']) &&
    ...
    protected function parseDateDefault($value, $time = false)
    {
        ...
        if ($time) {
            ...
            $td = $timedate->getNow(true);
            try {
                $td->modify($value);
            } catch (Throwable $t) {
                // ignore the modification
            }
            return $timedate->asDbDate($td);
        }
    }
     ...
     * @static
     */
    public function remove_relationship_meta($key, $db, $log, $tablename, $dictionary, $module_dir)
    {
     ...
     * @static
     */
    public function create_relationship_meta($key, &$db, &$log, $tablename, $dictionary, $module_dir)
    {
    ...
    public function get_importable_fields()
    {
        ...
        $this->ACLFilterFieldList($importableFields, [], ['min_access' => SugarACL::ACL_READ_WRITE]);
    ...
    public function erase(ErasureFieldList $fields, $check_notify)
    {
        ...
        $this->enqueueAuditedStateChanges(null);
    ...
    /**
     * Set the perform sugar action field for automate enable modules
     */
    private function setPerformSugarAction()
    {
        // if bean does not have perform_sugar_action field then do nothing
        if ($this->allowBatching) {
            return;
        }
        ...
        if (hasAutomateLicense()) {
            ...
            if ($sugar_config['customer_journey'] && $sugar_config['customer_journey']['enabled_modules']) {
                $enabledModules = $sugar_config['customer_journey']['enabled_modules'];
            }

            if (str_contains($enabledModules, $this->module_dir) &&
                CheckAndPerformRSA::canPerformRSA($this, $this->dataChanges)
            ) {
                // add entry in job queue
                ParentToSmartGuideRSA::createActivityRecordsAndJobQueue($this->id, $this->module_dir);
            }
        }
    }
    ...
    public function getAuditDataChanges(SugarBean $bean)
    {
        ...
        return $this->db->getDataChanges($bean, ['field_filter' => array_keys($audit_fields)]);
    }
    ...
    private function saveData($isUpdate, $check_notify)
    {
        ...
        $prev_date_modified = $this->date_modified ?? null;
        ...
        $this->call_custom_logic('before_save', [
    ...
    /**
     * Convert non-loaded currency fields
     */
    protected function prepareCurrencyFields()
    {
        ...
        foreach ($this->field_defs as $fieldName => $field) {
            if ($field['type'] === 'currency' && (float)($this->$fieldName ?? 0)) {
                if (!isset($field['currency_id']) && !isset($changes[$fieldName])) {
                    $this->$fieldName = SugarCurrency::convertAmount(
                        $this->$fieldName,
                        $changes['currency_id']['before'],
                        $this->currency_id
                    );
                }
            }
    ...
    protected function shouldUpdateTimeAwareSchedules()
    {
        ...
        if (is_null($tableExists)) {
            $tableExists = $this->db->tableExists($timeAwareSchedulesTableName);
        }
        return $tableExists && !empty($recalculationsVardefs);
    }
     ...
     * @return void
     */
    public function updateDependentField($filter_fields = null)
    {
    ...
    public function updateRelatedCalcFields($linkName = '')
    {
                    ...
                    if (!empty($beans)) {
                        foreach ($beans as $rBean) {
                            if (empty($rBean->deleted)) {
                                $relationship = $this->$lname->getRelationshipObject();
                                $lhslink = $relationship->getLHSLink();
                                $rBeanLinkName = $lhslink;

                                // Switch to the other side of the relationship when the names are the same because
                                // we want the other side when the link names are different
                                if ($lname == $lhslink) {
                                    $rBeanLinkName = $relationship->getRHSLink();
                                }

                                //Only update cache if the link is already loaded. On then can the cache need an update.
                                if (!empty($rBean->$rBeanLinkName)) {
                                    $rBean->$rBeanLinkName->updateBeanForSugarLogic($this);
                                }
        ...
        } elseif ($this->has_calc_field_with_link($linkName)) {
     ...
     * @return bool
     */
    public function has_calc_field_with_link($linkName = '')
    {
        $links = [];
        if (empty($linkName)) {
    ...
    public function send_assignment_notifications($notify_user, $admin)
    {
        if (($this->object_name == 'Meeting' || $this->object_name == 'Call')
            ...
            if (isset($this->current_notify_user->id)) {
                if (in_array($subject, $this->subjectsLog[$this->current_notify_user->id] ?? [], true)) {
                    return;
                }
                $this->subjectsLog[$this->current_notify_user->id][] = $subject;
            }
    ...
    protected function handle_remaining_relate_fields($exclude = [])
    {
        ...
        foreach ($this->field_defs as $def) {
            if ($def ['type'] == 'relate' && isset($def ['id_name']) && isset($def ['link'])) {
                ...
                } elseif (!empty($this->$linkField) && is_a($this->$linkField, 'Link2')) {
     ...
     */
    public function preprocess_encrypt_before_get($field)
    {
        if (!isset($this->encfield_touched)) {
            $this->encfield_touched = [];
        }
     ...
     * @deprecated Since 6.x
     */
    public function format_all_fields()
    {
        $GLOBALS['log']->deprecated('SugarBean.php: format_all_fields() is deprecated');
    }

    public function format_field($fieldDef)
    {
            ...
            switch ($def['type']) {
                ...
                case 'bool':
                    if (empty($this->$field)) {
                        ...
                        $this->$field = true;
                    } elseif (in_array(strval($this->$field), $boolean_false_values)) {
                        $this->$field = false;
                        $reformatted = true;
                    } else {
                        $this->$field = true;
     ...
     */
    public function fetch($id, array $fields = [], array $options = [])
    {
    ...
    public function fetchFromQuery(SugarQuery $query, array $fields = [], array $options = [])
    {
        ...
        [$queryFields, $additionalQueryFields, $secondaryFields] = $this->getQueryFields($query, $fields, $options);
        ...
        $this->call_custom_logic('before_fetch_query', ['query' => $query, 'fields' => $fields]);
        ...
        foreach ($rows as $row) {
            ...
            if (safeCount($additionalRows) && array_key_exists($row['id'], $additionalRows)) {
        ...
        $this->call_custom_logic('after_fetch_query', ['beans' => $beans, 'fields' => $fields, 'rows' => $rawRows]);
    ...
    protected function fetchFromAdditionalQuery(array $additionalQueryFields, array $rows)
    {
        ...
        if (safeCount($additionalQueryFields)) {
            $ids = [];
            foreach ($rows as $row) {
                $ids[] = $row['id'];
            }
            $additionalQuery = new SugarQuery();
            $additionalQuery->select(array_merge(['id'], $additionalQueryFields));
     ...
     * @return array
     */
    protected function getQueryFields(SugarQuery $query, array $fields = [], array $options = [])
    {
        $queryFields = [];
        ...
        return [$queryFields, $additionalQueryFields, $secondaryFields];
    }
    ...
    {
        $compensation = 0;
        $cntDbSet = safeCount($sqlRows);
        $cntBeanSet = safeCount($beans);
    ...
    public function populateFromRow(array $row, $convert = false, $getMoreData = true)
    {
        ...
        if (array_key_exists('erased_fields', $row)) {
            if (is_array($row['erased_fields'])) {
                ...
                $this->erased_fields = json_decode((string)$row['erased_fields'], true) ?: [];
            }
        }
        ...
        foreach ($this->field_defs as $field => $field_value) {
            ...
            if (array_key_exists($field_erased_fields, $row)) {
                $this->$field_erased_fields = json_decode((string)$row[$field_erased_fields], true) ?: [];
            }
    ...
    public function create_list_count_query($query, $params = [])
    {
        ...
        if (is_array($query)) {
            ...
            foreach ($query as $queryPart) {
                $results[] = $this->create_list_count_query($queryPart, $params);
            }
            return $results;
        }
    ...
    protected static function build_sub_queries_for_union($subpanel_list, $subpanel_def, $parentbean, $order_by)
    {
        ...
        foreach ($subpanel_list as $this_subpanel) {
            if (!$this_subpanel->isDatasourceFunction() || ($this_subpanel->isDatasourceFunction()
                ...
                $where_definition = preg_replace('/^\s*WHERE/i', '', (string)$query_array['where']);
                ...
                SugarACL::listFilter($submodule->module_dir, $acl_fields, ['bean' => $submodule, 'owner_override' => true], ['blank_value' => true]);
                foreach ($list_fields as $list_key => $list_field) {
        ...
        foreach ($fields as $field => $value) {
            ...
            if (empty($this->field_defs[$field]) || !empty($value['force_blank'])) {
                if (!empty($filter) && isset($filter[$field]['force_exists']) && $filter[$field]['force_exists']) {
                    ...
                    } else { //spaces are a fix for length issue problem with unions.  The union only returns the maximum number of characters from the first select statement.
                        ...
                        if (in_array('first_name', $data['db_concat_fields']) && in_array('last_name', $data['db_concat_fields'])) {
                            ...
                            if (preg_match($exp, $where, $matches)) {
                                ...
                                if (!empty($current_user) && $current_user->showLastNameFirst()) {
                                    $reversedFields = [];
                                    for ($i = safeCount($data['db_concat_fields']) - 1; $i >= 0; $i--) {
            ...
            if ($data['type'] == 'custom_query' && !empty($data['query_function'])) {
                $result = $this->callUserFunction($data['query_function'], $this, [$ret_array, $data]);
                if (!empty($result)) {
                    $ret_array = $result;
    ...
    public function process_list_query($query, $row_offset, $limit = -1, $max_per_page = -1, $where = '')
    {
        ...
        if (safeCount($data) > 0 && isset($queryParts['secondary_select'], $queryParts['secondary_from'])) {
        ...
        if (!empty($sugar_config['disable_count_query']) && !empty($limit)) {
            $rows_found = $row_offset + safeCount($list);
     ...
     */
    protected function callUserFunction($function, $current_bean, $execute_params = [])
    {
        if (!empty($function['function_class'])) {
            $execute_function = [$function['function_class'], $function['function_name']];
        } else {
        ...
        if (!empty($function['function_params'])) {
            if (empty($function['function_params_source']) || $function['function_params_source'] == 'parent') {
                $bean = $this;
            } elseif ($function['function_params_source'] == 'this') {
                $bean = $current_bean;
            } else {
                $bean = null;
            }
    ...
    public function process_union_list_query(
        ...
        if ($performSecondQuery) {
            ...
            if (!empty($sugar_config['disable_count_query']) && !empty($limit) && $limit != -1 && $limit != -99) {
                ...
                $rows_found = $index ?? $row_offset + safeCount($list);
    ...
    public function fill_in_link_field($idName, $relateFieldDef)
    {
            ...
            if (in_array($field['type'], static::$relateFieldTypes)) {
                ...
                if (isset($this->field_defs[$link]['link_type']) && $this->field_defs[$link]['link_type'] == 'one') {
                    ...
                    if (!empty($this->$link) || $this->load_relationship($link)) {
                        $params = $this->$link->beansAreLoaded() ? null : ['limit' => 1];
    ...
    private function fillInRelateField(array $definition): void
    {
        ...
        if ($this->retrieve_erased_fields) {
            if (isset($definition['link'])) {
                ...
                if (safeCount(array_intersect($relatedBean->erased_fields, $sourceFields)) > 0) {
    ...
    public function mark_undeleted($id)
    {
        ...
        $conn->executeStatement($query, [$this->deleted, $date_modified, $id]);
    ...
    public function build_related_list($query, $template, $row_offset = 0, $limit = -1)
    {
        ...
        while ($row = $this->db->fetchByAssoc($result)) {
            $record = BeanFactory::retrieveBean($template->module_name, $row['id'], ['disable_row_level_security' => $template->disable_row_level_security]);
            if (!empty($record)) {
    ...
    public function build_related_list_where($query, $template, $where, $in, $order_by, $limit = '', $row_offset = 0)
    {
        ...
        while ($row = $db->fetchByAssoc($result)) {
            $record = BeanFactory::retrieveBean($template->module_dir, $row['id'], ['disable_row_level_security' => $template->disable_row_level_security]);
            if (!empty($record)) {
                // this copies the object into the array
                $list[] = $record;
            }
     ...
     */
    public function updateDependentFieldForListView($listview_def_main = '', $filter_fields = null)
    {
        $listViewDefs = [];
        static $listview_def = '';
        static $module_name = '';
        ...
        if (!empty($listview_def_main)) {
            ...
            if (!empty($listview_meta_file)) {
                ...
                } elseif (isset($listViewDefs[$this->object_name])) {
     ...
     * @param boolean $deleted Optional, default true, if set to false deleted filter will not be added.
     * @param boolean $fieldLevelSearch Optional, default true. It marks whether we'll search fields with AND or OR
     * @return object Instance of this bean with fetched data.
     * @deprecated Use SugarQuery instead
     */
    public function retrieve_by_string_fields($fields_array, $encode = true, $deleted = true, $fieldLevelSearch = false)
    {
        $query = new \SugarQuery();
    ...
    public function getRelatedFields($module, $id, $fields, $return_array = false)
    {
        ...
        if (safeCount($selectFields) == 1 || empty($id)) {
            return '';
        }
     ...
     * @deprecated Use SugarQuery instead
     */
    public function add_team_security_where_clause(&$query, $table_alias = '', $join_type = 'INNER', $force_admin = false, $join_teams = false)
    {
    ...
    */
    ...
    public function set_relationship(
        ...
        if (!$check_duplicates || empty($row)) {
            $values = array_merge($data_values, $relate_values, [
                'id' => create_guid(),
    ...
    public function loadLayoutDefs()
    {
        ...
        if (empty($this->layout_def)) {
            foreach (SugarAutoLoader::existing(
                "modules/{$this->module_dir}/layout_defs.php",
                "custom/modules/{$this->module_dir}/Ext/Layoutdefs/layoutdefs.ext.php"
            ) as $file) {
                require $file;
            }
    ...
    */
    public function call_relationship_handler($target_base = 'module_dir', $return_handler = false)
    {
     ...
     */
    public function call_custom_logic($event, $arguments = [])
    {
     ...
     */
    public function process_workflow_alerts()
    {
    ...
    {
        $data = $GLOBALS['dictionary'][$this->object_name]['acls'] ?? [];
     ...
     * @param array $context
     */
    public function ACLFilterFields($view = 'detail', $context = [])
    {
     ...
     */
    public function ACLFieldAccess($field, $action = 'access', $context = [])
    {
        if (empty($context['bean'])) {
            $context['bean'] = $this;
        }
     ...
     */
    public function ACLFieldGet($field, $context = [])
    {
        if (empty($context['bean'])) {
            $context['bean'] = $this;
        }
    ...
    public function convertField($fieldvalue, $fieldDef)
    {
        if (!empty($fieldvalue)) {
            if (!(isset($fieldDef['source']) &&
                    !in_array($fieldDef['source'], ['db', 'custom_fields', 'relate'])
    ...
    public function create_export_query($order_by, $where)
    {
            ...
            if ($this->is_relate_field($field)) {
                ...
                if ($inWhere) {
                    ...
                    if (empty($fields_to_exclude[$module_name_lower])) {
                        $fields_to_exclude[$module_name_lower] = [];
                    }
        ...
        $relatedNameFields = [
            ['name' => 'assigned_user_name', 'alias' => 'assigned_user', 'id' => 'assigned_user_id'],
            ['name' => 'modified_by_name', 'alias' => 'modified_by_user', 'id' => 'modified_user_id'],
            ['name' => 'created_by_name', 'alias' => 'created_by_user', 'id' => 'created_by'],
        ];
    ...
    protected function loadDuplicateCheckManager()
    {
        if (empty($this->duplicate_check_manager)) {
            if (isset($GLOBALS['dictionary'][$this->object_name]['duplicate_check']) &&
                ...
                $data = [];  // duplicate_check not defined  or  is not enabled
            }
     ...
     * @return array
     */
    public function getRelateFieldQuery($field_def, $joinTableAlias, $selectedFields = [])
    {
        global $locale;
     ...
     * @return bool
     */
    public function populateFetchedEmail($populate = 'fetched_row')
    {
    ...
    public function getLockedFields()
    {
        ...
        foreach ($relBeans as $relBean) {
            $merge = empty($relBean->pro_locked_variables) ? [] : json_decode($relBean->pro_locked_variables);
    ...
    {
        return $this->field_defs['locked_fields']['link'] ?? '';
    }
    ...
    public function isEmailNotificationNeeded()
    {
        ...
        if (isset($this->fetched_row['assigned_user_id'])) {
            ...
            $old_assigned_user_id = CalendarEventsUtils::getInstance()->getOldAssignedUser(
                $this->module_dir,
                $this->id
            );
        }
    ...
    /**
     * Hook to adjust module vardefs data before it is returned from the
     * MetadataManager
     *
    ...
    /**
     * Enqueues updates to the audit log with the changes from the last audited state to the current one.
     *
     ...
     * @throws DBALException
     */
    public function enqueueAuditedStateChanges(?Subject $overrideSubject): void
    {
        if (!$this->is_AuditEnabled()) {
            return;
        }
        ...
        if (safeCount($changes) < 1) {
            return;
        }

        $this->auditedStateQueue[] = [
            // can subject or this->context change during between placing in queue and commit?
    ...
    public function commitAuditedStateChanges(?Subject $overrideSubject): void
    {
        ...
        $this->enqueueAuditedStateChanges($overrideSubject);

        $eventRepository = $this->getEventRepository();
        foreach ($this->auditedStateQueue as ['subject' => $subject, 'changes' => $changes]) {
            if ($subject) {
                $auditEventId = $eventRepository->registerUpdateAttributedToSubject($this, $subject);
            } else {
        ...
        try {
            ...
            if (array_key_exists('erased_fields', $row)) {
                if (is_array($row['erased_fields'])) {
                    ...
                    $erasedFields = json_decode((string)$row['erased_fields'], true) ?: [];
                }
            }
