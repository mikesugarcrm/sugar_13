...
// @codingStandardsIgnoreEnd
    ...
    public function get_start_end_date_filter($layout_def, $begin, $end)
    {
        if (isset($layout_def['rel_field'])) {
            $field_name = $this->reporter->db->convert(
                ...
                'CONCAT',
                ["' '", $this->reporter->db->convert($layout_def['rel_field'], 'time_format')]
            );
        } else {
    ...
    public function queryFilterTP_last_n_days($layout_def)
    {
    ...
    public function queryFilterTP_next_n_days($layout_def)
    {
    ...
    public function displayListPlain($layout_def)
    {
        ...
        if (safeCount(explode('-', $content)) == 2) {
            return $content;
            // if date field
    ...
    public function displayList($layout_def)
    {
        ...
        $content = parent::_get_list_value($layout_def);

        if ($layout_def['type'] === 'datetime' || $layout_def['type'] === 'datetimecombo') {
            $td = $timedate->to_display_date_time($content, null, false);

            return $td;
        } elseif ($layout_def['type'] === 'date') {
            $td = $timedate->to_display_date($content, false);
    ...
    {
        return in_array($type, ['datetime', 'datetimecombo']);
    }
    ...
    public function queryGroupByFiscalYear($layout_def)
    {
        ...
        $query = $this->reporter->db->convert($date, 'date_format', ['%Y']);
    ...
    public function querySelectmonth($layout_def)
    {
        ...
        return $this->reporter->db->convert($return, 'date_format', ['%Y-%m']) . ' ' . $this->_get_column_alias($layout_def) . "\n";
    }
    ...
    public function queryGroupByMonth($layout_def)
    {
        ...
        return $this->reporter->db->convert($return, 'date_format', ['%Y-%m']) . "\n";
    }
    ...
    public function queryOrderByMonth($layout_def)
    {
        ...
        $orderBy = $this->reporter->db->convert($return, 'date_format', ['%Y-%m']);
    ...
    public function querySelectday($layout_def)
    {
        ...
        return $this->reporter->db->convert($return, 'date_format', ['%Y-%m-%d']) . ' ' . $this->_get_column_alias($layout_def) . "\n";
    }
    ...
    public function queryGroupByDay($layout_def)
    {
        ...
        return $this->reporter->db->convert($return, 'date_format', ['%Y-%m-%d']) . "\n";
    }
    ...
    public function querySelectyear($layout_def)
    {
        ...
        return $this->reporter->db->convert($return, 'date_format', ['%Y']) . ' ' . $this->_get_column_alias($layout_def) . "\n";
    }
    ...
    public function queryGroupByYear($layout_def)
    {
        ...
        return $this->reporter->db->convert($return, 'date_format', ['%Y']) . "\n";
    }
    ...
    public function querySelectquarter($layout_def)
    {
        ...
        return $this->reporter->db->convert(
            $this->reporter->db->convert($column, 'date_format', ['%Y']),
            'CONCAT',
            ["'-'", $this->reporter->db->convert($column, 'quarter')]
        )
    ...
    public function displayInput($layout_def)
    {
        ...
        $filterTypes = [' ' => $app_strings['LBL_NONE'],
