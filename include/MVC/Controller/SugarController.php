...
class SugarController
{
     ...
     */
    protected string $do_action = 'index';
    /**
    ...
    private function loadMapping($var, $merge = false)
    {
        ...
        if (!${$var}) {
            ...
            $varname = str_replace(' ', '', ucwords(str_replace('_', ' ', (string)$var)));
    ...
    {
        if (!isset($this->view_object_map['remap_action']) && isset($this->action_view_map[strtolower((string)$this->action)])) {
            $this->view_object_map['remap_action'] = $this->action_view_map[strtolower((string)$this->action)];
        }
    ...
    public function pre_save()
    {
        ...
        foreach ($this->bean->field_defs as $field => $properties) {
            ...
            $sf = $sfh->getSugarField(ucfirst((string)$type), true);
            if (isset($_POST[$field])) {
            ...
            } elseif (!empty($properties['isMultiSelect']) && !isset($_POST[$field]) && isset($_POST[$field . '_multiselect'])) {
                $this->bean->$field = '';
            }
            if ($sf != null) {
    ...
    {
        $searchEngine = SugarSearchEngineFactory::getInstance('', [], true);
    ...
    protected function action_massupdate()
    {
        ...
        if (!empty($_REQUEST['massupdate']) && $_REQUEST['massupdate'] == 'true' && (!empty($_REQUEST['uid']) || !empty($_REQUEST['entire']))) {
            ...
            $module = $this->request->getValidInputRequest('module', 'Assert\Mvc\ModuleName');
            $query = unserialize(base64_decode((string)$_REQUEST['current_query_by_page']), ['allowed_classes' => false]);
            ...
            if (!empty($_POST['mass'])) {
                $total_records = safeCount($_POST['mass']);
                $failed_update = safeCount($arr);
                ...
                if ($successful_update == $total_records) {
                    ...
                    if (!empty($_REQUEST['Delete'])) {
                        ...
                        $massupdate_status = str_replace('{{num}}', $successful_update, (string)$massupdate_status);
                    } else {
            ...
            if ($_REQUEST['return_module'] == 'Emails') {
                if (!empty($_REQUEST['type']) && !empty($_REQUEST['ie_assigned_user_id'])) {
                    $this->req_for_email = ['type' => $_REQUEST['type'], 'ie_assigned_user_id' => $_REQUEST['ie_assigned_user_id']]; // Specifically for My Achieves
                }
            }
            $_REQUEST = $query;
    ...
    private function remapWirelessAction()
    {
        ...
        if ($this->module == 'Home' && $this->action == 'index' && isset($_SESSION['isMobile'])) {
            ...
            if ($action == 'editview') {
                $this->action = 'wirelessedit';
            }
            if ($action == 'detailview') {
                $this->action = 'wirelessdetail';
            }
            if ($action == 'listview') {
                $this->action = 'wirelesslist';
            }
        } else {
            if ($action == 'wirelessedit') {
                $this->action = 'editview';
            }
            if ($action == 'wirelessdetail') {
                $this->action = 'detailview';
            }
            if ($action == 'wirelesslist') {
                $this->action = 'listview';
            }
            if ($action == 'wirelessmodule') {
                $this->action = 'listview';
            }
        }
        $this->do_action = $this->action;
