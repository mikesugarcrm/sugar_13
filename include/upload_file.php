...
class UploadFile
{
    ...
    public static function duplicate_file($old_id, $new_id, $file_name = '')
    {
        ...
        if (is_dir($source)) {
            $GLOBALS['log']->error("{$source} is a directory.");
        ...
        if (!file_exists($source)) {
            ...
            if (file_exists($oldStyleSource)) {
                // change to new style
                if (is_dir($oldStyleSource)) {
                    $GLOBALS['log']->error("{$oldStyleSource} is a directory.");
        ...
        if ($_FILES[$this->field_name]['error'] != UPLOAD_ERR_OK) {
            if ($_FILES[$this->field_name]['error'] != UPLOAD_ERR_NO_FILE) {
                if ($_FILES[$this->field_name]['error'] == UPLOAD_ERR_INI_SIZE) {
                    ...
                    $errMess = string_format($GLOBALS['app_strings']['UPLOAD_ERROR_TEXT_SIZEINFO'], [$_FILES[$this->field_name]['error'], self::$filesError[$_FILES[$this->field_name]['error']], $sugar_config['upload_maxsize']]);
                    ...
                    $errMess = string_format($GLOBALS['app_strings']['UPLOAD_ERROR_TEXT'], [$_FILES[$this->field_name]['error'], self::$filesError[$_FILES[$this->field_name]['error']]]);
                    $this->setError('fatal', $errMess, $_FILES[$this->field_name]['error']);
                }
            }
            return false;
     ...
     * @return bool True on success
     */
    public function final_move($bean_id, $temporary = false)
    {
