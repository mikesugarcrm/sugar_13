...
class SearchForm
{
    ...
    public function __construct($seed, $module, $action = 'index', $options = [])
    {
        ...
        $this->module = $module;
        $this->action = $action;
        $this->tabs = [['title' => $GLOBALS['app_strings']['LNK_BASIC_SEARCH'],
    ...
    public function setup(array $searchdefs, array $searchFields, string $tpl, string $displayView = 'basic_search', ?array $listViewDefs = []): void
    {
        ...
        if ($this->showAdvanced) {
            $this->nbTabs++;
            $this->tabs[] = ['title' => $GLOBALS['app_strings']['LNK_ADVANCED_SEARCH'],
        ...
        if (isset($this->showCustom) && is_array($this->showCustom)) {
            foreach ($this->showCustom as $v) {
                $this->nbTabs++;
                $this->tabs[] = ['title' => $GLOBALS['app_strings']['LNK_' . strtoupper($v)],
                    'link' => $this->module . '|' . $v,
                    'key' => $this->module . '|' . $v,
    ...
    public function display($header = true)
    {
        ...
        SugarACL::listFilter(
            ...
            $this->fieldDefs,
            ['owner_override' => true],
            ['use_value' => true, 'suffix' => '_' . $this->parsedView, 'add_acl' => true]
        );
        ...
        foreach ($this->tabs as $tabkey => $viewtab) {
            $viewName = str_replace([$this->module . '|', '_search'], '', $viewtab['key']);
    ...
    public function displaySavedSearch()
    {
    ...
    public function _displayTabs($currentKey)
    {
        ...
        $params = ['displayColumns', 'hideTabs', 'orderBy', 'sortOrder'];
        $values = $script = [];
        ...
        if (!empty($this->searchdefs['layout'][$this->displayView])) {
            foreach ($this->searchdefs['layout'][$this->displayView] as $data) {
                if (is_array($data)) {
                    ...
                    $this->formData[] = ['field' => ['name' => $data . '_' . $this->parsedView]];
                }
            }
        }
            ...
            foreach ($this->seed->toArray() as $name => $value) {
                ...
                if (!is_scalar($this->fieldDefs[$fvName]['options'] ?? null)) {
                    ...
                    if ($this->fieldDefs[$fvName]['type'] == 'parent_type' || $this->fieldDefs[$fvName]['type'] == 'parent' || (isset($this->searchFields[$name]['options_add_blank']) && $this->searchFields[$name]['options_add_blank'])) {
                        $this->fieldDefs[$fvName]['options'] = array_merge(['' => ''], $this->fieldDefs[$fvName]['options']);
                    }
                }
                    ...
                    if (!empty($this->fieldDefs[$fvName]['function']['returns']) && $this->fieldDefs[$fvName]['function']['returns'] == 'html') {
                        ...
                        if (isset($this->fieldDefs[$fvName]['function_bean'])) {
                            ...
                            if (method_exists($funcBean, $function_name)) {
                                $function_name = [$funcBean, $function_name];
                            }
                        }
        ...
        if ((!empty($array['searchFormTab']) || !empty($switchVar)) && !empty($this->searchFields)) {
            ...
            if ($switchVar == 'saved_views') {
                    ...
                    foreach ($this->seed->field_defs as $key => $params) {
                        if ($key != 'assigned_user_name' && $key != 'modified_by_name') {
                            ...
                            if (in_array($key . '_' . $SearchName, $arrayKeys) && !in_array($key, $searchFieldsKeys)) {
                                $this->searchFields[$key] = ['query_type' => 'default', 'value' => $array[$long_name]];
                                ...
                                if (!empty($params['type']) && $params['type'] == 'parent'
                                    ...
                                    $this->searchFields[$params['type_name']] = ['query_type' => 'default',
                ...
                if (isset($array['team_name_advanced_new_on_update'])) {
                    ...
                    $this->searchFields['team_set_id'] = $sf->getTeamSetIdSearchField('team_name_advanced', $teamSearchType, [], $array);
                    if (isset($array['primary_team_name_advanced_collection'])) {
                        $this->searchFields['team_id'] = $sf->getTeamIdSearchField('primary_team_name_advanced_collection', 'any', [], $array);
                    }
                }
            }
        }
        ...
        foreach (safeIsIterable($this->searchFields) ? $this->searchFields : [] as $field => $parms) {
            ...
            if (!empty($parms['enable_range_search']) && empty($type)) {
                if (preg_match('/^start_range_(.*?)$/', $field, $match)) {
                    ...
                    if (in_array($parms['operator'], ['=', 'between', 'not_equal', 'less_than', 'greater_than', 'less_than_equals', 'greater_than_equals'])) {
                        $field_type = $this->seed->field_defs[$real_field]['type'] ?? '';
                            ...
                            case 'like':
                                if ($type == 'bool' && $field_value == 0) {
                                    ...
                                    if ($UnifiedSearch && strpos($field_value, ' ') !== false && strpos(
                                        $db_field,
                                        (string) $parms['db_field'][safeCount($parms['db_field']) - 1]
                                    ) !== false) {
                                        ...
                                        for ($i = 0; $i < safeCount($concat_fields); $i++) {
     ...
     */
    private function isEmptyDropdownField($name = '', $value = [])
    {
    ...
    public static function retrieveSearchDefs($module)
    {
        ...
        return ['searchdefs' => $searchdefs, 'searchFields' => $searchFields];
    }
}
