...
function process_action_update($focus, $action_array)
{
    ...
    foreach ($action_array['basic'] as $field => $new_value) {
        if (empty($action_array['basic_ext'][$field])) {
            ...
            if (in_array($focus->field_defs[$field]['type'], ['double', 'decimal', 'currency', 'float'])) {
    ...
    if (!empty($rel_list[0])) {
        ...
        foreach ($rel_list as $rel_object) {
            ...
            foreach ($action_array['basic'] as $field => $new_value) {
                ...
                //otherwise rely on the basic_ext to handle the action for this field
                if ($field == 'email1') {
                    $rel_object->email1_set_in_workflow = $rel_object->email1;
                }
                if ($field == 'email2') {
                    $rel_object->email2_set_in_workflow = $rel_object->email2;
                }
            ...
            if ($old_owner != $rel_object->assigned_user_id) {
                $check_notify = true;
            }
            if (!empty($_REQUEST['massupdate']) && $_REQUEST['massupdate'] == 'true') {
                $check_notify = false;//if in a massupdate, the notification will not be sent, because it will take a long time.
            }
...
function workflow_convert_multienum_value($value)
{
    ...
    return encodeMultienumValue([$value]);
}
