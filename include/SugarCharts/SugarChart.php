...
class SugarChart
{
    ...
    public function __construct()
    {
        ...
        $this->chart_properties['gauge_target_list'] = [];
    ...
    public function calculateTotal($group_by)
    {
        ...
        for ($i = 0; $i < safeCount($this->data_set); $i++) {
    ...
    public function processGauge($position, $target, $phases = [])
    {
        if (empty($phases)) {
            $phases[translate('LBL_PHASE') . ' 1'] = ['start' => 0, 'end' => ceil($target / 4),];
            $phases[translate('LBL_PHASE') . ' 2'] = ['start' => ceil($target / 4), 'end' => ceil($target / 2),];
            $phases[translate('LBL_PHASE') . ' 3'] = ['start' => ceil(($target / 2)), 'end' => ceil(($target / 4) * 3),];
            $phases[translate('LBL_PHASE') . ' 4'] = ['start' => ceil(($target / 4) * 3), 'end' => ceil($target),];
        }
    ...
    public function sortData($data_set, $keycolname1 = null, $translate1 = false, $keycolname2 = null, $translate2 = false, $ifsort2 = false)
    {
        ...
        if (isset($sortby1[0]) && $sortby1[0] == []) {
            unset($sortby1[0]);//the beginning of month after search is blank.
        }
