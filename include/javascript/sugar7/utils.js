...
(function(app) {
    app.events.on("app:init", function() {
        ...
        app.utils = _.extend(app.utils, {
            ...
            /**
             * Setting fields editability options for IDM
             *
             * @param {Array} fields list of fields objects
             */
            setIDMEditableFields: function(fields, tplName) {
                let idmException = [
                    'email1',
                    'email',
                    'email_addresses',
                ...
                _.each(fields, function(field) {
                    ...
                    if (!app.config.idmModeEnabled) {
                        let adminDeveloperReadonlyFilds = ['UserType'];
                        let regularUserReadonlyFilds = [
                            'user_name',
                            'status',
                            'UserType',
                            'license_type',
                        ...
                        //for Admin & Developer
                        if (isAdmin && isDev && _.contains(adminDeveloperReadonlyFilds, field.name)) {
                            field.readonly = true;
                            return;
                        }

                        //for regular user
                        if (!(isAdmin || isDev) && _.contains(regularUserReadonlyFilds, field.name)) {
                            field.readonly = true;
                            return;
                        }
                    ...
                    if (_.contains(readonlyFilds, field.name) || field.idm_mode_disabled) {
                        field.readonly = true;
