...
abstract class MysqlManager extends DBManager
{
    ...
    protected $type_range = [
        ...
        'uint' => ['min_value' => 0, 'max_value' => 4294967295],
        'ulong' => ['min_value' => 0, 'max_value' => 18446744073709551615],
        'long' => ['min_value' => -9223372036854775808, 'max_value' => 9223372036854775807],
        'short' => ['min_value' => -32768, 'max_value' => 32767],
        'tinyint' => ['min_value' => -128, 'max_value' => 127],
    ];
...
WHERE TABLE_SCHEMA = ?
    ...
    protected function oneColumnSQLRep($fieldDef, $ignoreRequired = false, $table = '', $return_as_array = false, $action = null)
    {
        ...
        if (isset($ref['default']) && in_array($ref['colBaseType'], ['text', 'blob', 'longtext', 'longblob'])) {
            $ref['default'] = '';
        }
...
EOQ;
    ...
    public function canInstall()
    {
        ...
        if (version_compare($db_version, '4.1.2') < 0) {
            return ['ERR_DB_MYSQL_VERSION', $db_version];
        }
        return true;
    }
