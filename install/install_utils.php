 ...
 */
function installerHook($function_name, $options = [])
{
...
{
    if (isset($_SESSION['INSTALLED_LANG_PACKS']) && safeCount($_SESSION['INSTALLED_LANG_PACKS']) > 0) {
...
function getInstalledLangPacks($showButtons = true)
{
    ...
    if (isset($_SESSION['INSTALLED_LANG_PACKS']) && !empty($_SESSION['INSTALLED_LANG_PACKS'])) {
        if (safeCount($_SESSION['INSTALLED_LANG_PACKS']) > 0) {
...
{
    return DBManagerFactory::getTypeInstance($_SESSION['setup_db_type'], ['db_manager' => $_SESSION['setup_db_manager']]);
}
...
function handleSugarConfig()
{
    ...
    if (isset($_SESSION['INSTALLED_LANG_PACKS']) && ArrayFunctions::is_array_access($_SESSION['INSTALLED_LANG_PACKS']) && !empty($_SESSION['INSTALLED_LANG_PACKS'])) {
        foreach ($_SESSION['INSTALLED_LANG_PACKS'] as $langZip) {
            ...
            if (!empty($lang)) {
                $exLang = explode('::', $lang);
                if (is_array($exLang) && safeCount($exLang) == 3) {
...
function handleWebConfig($iisCheck = true)
{
    ...
    $redirect_config_array = [
        ['1' => '^portal$', '2' => 'portal/'],
    ];

    $rewrite_config_array = [
        [
    ...
    for ($i = 0; $i < safeCount($redirect_config_array); $i++) {
        $xmldoc->startElement('rule');
    ...
    for ($i = 0; $i < safeCount($rewrite_config_array); $i++) {
        $xmldoc->startElement('rule');
...
function pullSilentInstallVarsIntoSession()
{
    ...
    $config_subset = [
        ...
        'setup_db_options' => !empty($sugar_config['dbconfigoptions']) ? $sugar_config['dbconfigoptions'] : [],
    ];
 ...
 */
function getLangPacks($display_commit = true, $types = ['langpack'], $notice_text = '')
{
    global $mod_strings;
    global $next_step;
...
function createWebAddress()
{
    ...
    $one = $seed[random_int(0, safeCount($seed) - 1)];
    $two = $seed[random_int(0, safeCount($seed) - 1)];
    $tld = $tlds[random_int(0, safeCount($tlds) - 1)];
...
function createEmailAddress()
{
    ...
    $part[0] = $seed[random_int(0, safeCount($seed) - 1)];
    $part[1] = $seed[random_int(0, safeCount($seed) - 1)];
    $part[2] = $seed[random_int(0, safeCount($seed) - 1)];
...
function addDefaultRoles($defaultRoles = [])
{
    global $db;
        ...
        foreach ($role as $category => $actions) {
            foreach ($actions as $name => $access_override) {
                if ($name == 'fields') {
                    ...
                    $stmt = $conn->executeQuery($queryACL, [$category, $name]);
