...
class Bug39757Test extends TestCase
{
    ...
    {
        $sql = "DELETE FROM meetings WHERE id = '{$this->meetingId}'";
        $GLOBALS['db']->query($sql);
    ...
    {
        $meetingsSql = "UPDATE meetings AS a INNER JOIN meetings AS b ON a.id = b.id SET a.date_end = date_add(b.date_start, INTERVAL + concat(b.duration_hours, b.duration_minutes) HOUR_MINUTE) WHERE a.id = '{$this->meetingId}'";
        $GLOBALS['db']->query($meetingsSql);

        $meeting = new Meeting();
        $meeting->disable_row_level_security = true;
        $meeting->retrieve($this->meetingId);
