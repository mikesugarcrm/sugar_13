<?php
/*
 * Your installation or use of this SugarCRM file is subject to the applicable
 * terms available at
 * http://support.sugarcrm.com/Resources/Master_Subscription_Agreements/.
 * If you do not agree to all of the applicable terms or do not have the
 * authority to bind the entity as an authorized representative, then do not
 * install or use this SugarCRM file.
 *
 * Copyright (C) SugarCRM Inc. All rights reserved.
 */
declare(strict_types=1);

use Regression\Helpers\MLPBuilder;
use Regression\SugarCRMAssessment;

class UpgradeWizardManifestAssessment extends SugarCRMAssessment
{
    public function getAssessmentDescription(): string
    {
        return 'PHP Object injection through manifest.php file in UpgradeWizard';
    }

    public function run(): void
    {
        $mlpBuilder = (new MLPBuilder(static::$sugarVersion['sugar_version'], 'manifest_shell'))
            ->addFile(
                'manifest.php',
                <<<'PHP'
                <?php unserialize(base64_decode('')); ?>
                PHP,
                'manifest.php'
            )
            ->build();
        $this->login('admin', 'asdf')
            ->bwcLogin()
            ->uploadMLP($mlpBuilder->getPath())
            ->assumeSubstring('Manifest contains forbidden expression', $validationFailed)
            ->get('/custom/shell.php')
            ->assumeSubstring('____', $shellHasBeenUploaded)
            ->checkAssumptions('Manifest shell scripting was found.', !$validationFailed, $shellHasBeenUploaded);
    }
}
