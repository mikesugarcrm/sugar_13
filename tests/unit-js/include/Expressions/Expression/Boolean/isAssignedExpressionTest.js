 ...
 */
describe('isAssigned Function', function() {
    let app;
    let oldApp;
    let dm;
    let sinonSandbox;
    ...
    it('should return true if a record is assigned', function() {
        model.set('assigned_user_id', App.user.id);
        let res = new SUGAR.expressions.IsAssignedExpression([], getSLContext(model));
        expect(res.evaluate()).toBe('true');
    });

    it('should return false if a record is not assigned', function() {
