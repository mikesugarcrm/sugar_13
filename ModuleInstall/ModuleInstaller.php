...
class ModuleInstaller
{
     ...
     * @param array $modules
     */
    public function rebuildExt($ext, $filename, $modules = [])
    {
    ...
    public function getSidecarFileInfo($path = '')
    {
        ...
        if (preg_match($regExPattern, $path, $matches) && safeCount($matches) === 4) {
    ...
    public function uninstall_global_search()
    {
        ...
        foreach ($this->installdefs['beans'] as $beanDefs) {
            ...
            foreach ($users as $userId => $userName) {
                ...
                } elseif (empty($prefsInSession['search'])) {
    ...
    public function rebuild_extensions($modules = [], $filter = [])
    {
        foreach ($this->extensions as $extname => $ext) {
            if (empty($filter) || in_array($extname, $filter)) {
                ...
                if ($func == 'rebuild_languages') {
                    $this->rebuild_languages([], $modules);
        ...
        if (isset($this->installdefs['language'])) {
            ...
            foreach ($this->installdefs['language'] as $packs) {
                ...
                if (!file_exists($lang_file)) {
                    ...
                    $contents = $this->getExtensionFileContents([$lang_file, $temp_lang_file]);
    ...
    public function uninstall_languages()
    {
        ...
        if (isset($this->installdefs['language'])) {
            ...
            foreach ($this->installdefs['language'] as $packs) {
                ...
                } elseif (sugar_is_file($path . '/' . DISABLED_PATH . '/' . $packs['language'] . '.' . $this->id_name . '.php', 'w')) {
 ...
 */
    ...
    public function install_relationship($file)
    {
        ...
        array_walk($rel_dictionary, ['ModuleInstaller', 'cleanUpRelationship']);
    ...
    public function uninstall_relationship($file, $rel_dictionary = null)
    {
        ...
        foreach ($rel_dictionary as $rel_name => $rel_data) {
            ...
            foreach ($mods as $mod) {
                ...
                foreach ([$rel_name . '_' . $mod . '.php', $rel_name . 'MetaData.php'] as $fileName) {
    ...
    public function uninstall($base_dir)
    {
        ...
        $total_steps += safeCount($tasks); //now the real number of steps
        if (file_exists($this->base_dir . '/manifest.php')) {
            if (!$this->silent) {
                $current_step++;
                display_progress_bar('install', $current_step, $total_steps);
    ...
    public function rebuild_vardefs($modules = [])
    {
    ...
    public function rebuild_relationships($changedModules = [])
    {
     ...
     */
    public function rebuild_all($silent = false, $modules = [])
    {
        if (defined('TEMPLATE_URL')) {
            SugarTemplateUtilities::disableCache();
        }
    ...
    public function reset_file_cache()
    {
        // check for upgrades
    ...
    protected function sortExtensionFiles(array $files, string $orderMapFile)
    {
        ...
        if (safeCount($filesSortedByMtime) < 2) {
     ...
     */
    public function merge_files($path, $name, $filter = '', $application = false, $modules = [])
    {
        if (!$application) {
        ...
        foreach ($layoutAdditions as $deployedModuleName => $fieldName) {
            if (!in_array(strtolower($deployedModuleName), $invalidModules)) {
                ...
                if (isModuleBWC($deployedModuleName)) {
                    $views = [MB_EDITVIEW, MB_DETAILVIEW];
                } else {
                    $views = [MB_RECORDVIEW];
                }
    ...
    protected function enableRelationship($relName, $relationship, $enable)
    {
        ...
        foreach ($mods as $mod) {
            ...
            foreach ([$filename, 'custom' . $filename, $relName . '_' . $mod . '.php'] as $fn) {
                foreach (['Vardefs', 'Layoutdefs', 'WirelessLayoutdefs'] as $path) {
    ...
    public function __call($name, $args)
    {
        ...
        if (safeCount($nameparts) == 2 && isset($this->extensions[$nameparts[1]])) {
    ...
    public static function getPortalConfig()
    {
        ...
        $portalConfig = [
            ...
            'serverUrl' => $config->get('site_url') . '/rest/v11_23',
            ...
            'allowedLinkSchemes' => $config->get('allowed_link_schemes', []),
            'enableSelfSignUp' => $enableSelfSignUp,
        ];

        $jsConfig = $config->get('additional_js_config', []);
    ...
    public static function getBaseConfig()
    {
        ...
        $sidecarConfig = [
            ...
            'allowedLinkSchemes' => $config->get('allowed_link_schemes', []),
            'ajaxDebounceInterval' => $config->get('ajax_debounce_interval', 500),
            'emailParticipantsMinSearch' => $config->get('email_participants_min_search', 1),
        ];
     ...
     */
    protected function clearAffectedVardefsCache($modules = [])
    {
        foreach ($modules as $module) {
