...
class SugarUpgradeMerge7Templates extends UpgradeScript
{
    ...
    public function mergePanelDefs($old_viewdefs, $new_viewdefs, $custom_viewdefs)
    {
        ...
        foreach ($added_fields as $field => $data) {
            ...
            // replace field defs for users
            // special case for users bwc to sidecar upgrade
            if ($this->moduleName === 'Users' && version_compare($this->from_version, '13.2.0', '<=')) {
                $custom_viewdefs = $this->replaceFieldDef($custom_viewdefs, $custom_fields, $added_fields, $field);
            }
        ...
        if (!empty($changed_fields)) {
            ...
            $this->log('Custom Fields: ' . var_export($custom_fields, true));
            $this->log('Old Fields: ' . var_export($old_fields, true));
            ...
            foreach ($changed_fields as $field => $data) {
                ...
                if ($custom_fields[$field]['data'] == $old_fields[$field]['data']) {
                    $custom_viewdefs = $this->replaceFieldDef($custom_viewdefs, $custom_fields, $new_fields, $field);
                }

                // replace field defs for users
                // special case for bwc to sidecar upgrade
                if ($this->moduleName === 'Users' && version_compare($this->from_version, '13.2.0', '<=')) {
                    $custom_viewdefs = $this->replaceFieldDef($custom_viewdefs, $custom_fields, $new_fields, $field);
                }
            }
            $this->log('Custom viewdefs: ' . var_export($custom_viewdefs, true));
        }
    ...
    /**
     * Replace field definition from custom view defs with new field def
     *
     * @param array $customViewDefs
     * @param array $customFields
     * @param array $newFields
     * @param string $field
     * @return array
     */
    protected function replaceFieldDef(array $customViewDefs, array $customFields, array $newFields, string $field): array
    {
        $pindex = $customFields[$field]['pindex'];
        $findex = $customFields[$field]['findex'];
        $customViewDefs[$this->moduleName][$this->clientType]['view'][$this->viewName]['panels'][$pindex]['fields'][$findex] = $newFields[$field]['data'];
        $this->needSave = true;
        return $customViewDefs;
    }
