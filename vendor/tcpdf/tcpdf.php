...
if (!class_exists('TCPDF', false)) {
    ...
    class TCPDF
    {
         ...
         */
        protected $cache_UTF8StringToArray = [];
        ...
        protected function checkPageBreak($h = 0, $y = '')
        {
            ...
            if ((int)$y + (int)$h > (int)$this->PageBreakTrigger && !$this->InFooter && $this->AcceptPageBreak()) {
                //Automatic page break
                $x = $this->x;
        ...
        public function Image($file, $x = '', $y = '', $w = 0, $h = 0, $type = '', $link = '', $align = '', $resize = false, $dpi = 300, $palign = '', $ismask = false, $imgmask = false, $border = 0, $fitbox = false)
        {
            ...
            $sizeDetector = new SizeDetector([$GLOBALS['sugar_config']['host_name']]);
            $imsize = $sizeDetector->getSize($file);
            if ($imsize === FALSE) {
                $this->Error($sizeDetector->getError());
            }
        ...
        public function Output($name = 'doc.pdf', $dest = 'I')
        {
            ...
            if ($dest != 'F') {
                ...
                $name = preg_replace('/[^a-zA-Z0-9\._\-\x{4e00}-\x{9fa5}\x{a700}-\x{d7af}]/u', '', $name);
            }
            if ($this->sign) {
        ...
        protected function _putannots($n)
        {
            if (isset($this->PageAnnots[$n])) {
                ...
                foreach ($this->PageAnnots[$n] as $key => $pl) {
                    ...
                    //$annots .= ' /AS ';
                    $annots .= ' /Border [';
                    if (isset($pl['opt']['border']) and (safeCount($pl['opt']['border']) >= 3)) {
        ...
        protected function _RC4($key, $text)
        {
            if ($this->last_rc4_key != $key) {
                $k = str_repeat($key, floor(((256 / strlen($key)) + 1)));
                $rc4 = range(0, 255);
                $j = 0;
                for ($i = 0; $i < 256; ++$i) {
                    $t = $rc4[$i];
        ...
        public function SetProtection($permissions = [], $user_pass = '', $owner_pass = null)
        {
            ...
            if ($owner_pass === null) {
                $owner_pass = uniqid(random_int(0, mt_getrandmax()));
            }
            $this->encrypted = true;
		...
		*/
         ...
         * @access public
         */
        public function CoonsPatchMesh($x, $y, $w, $h, $col1 = [], $col2 = [], $col3 = [], $col4 = [], $coords = [0.00, 0.0, 0.33, 0.00, 0.67, 0.00, 1.00, 0.00, 1.00, 0.33, 1.00, 0.67, 1.00, 1.00, 0.67, 1.00, 0.33, 1.00, 0.00, 1.00, 0.00, 0.67, 0.00, 0.33], $coords_min = 0, $coords_max = 1)
        {
            $patch_array = [];
            $this->Clip($x, $y, $w, $h);
            ...
            for ($i = 0; $i < $count_patch; ++$i) {
                ...
                $count_points = safeCount($patch_array[$i]['points']);
                ...
                $count_cols = safeCount($patch_array[$i]['colors']);
            ...
            while ($elkey < $maxel) {
                ...
                if (preg_match($tagpattern, $element)) {
                    ...
                    if ($element[0] == '/') {
                        ...
                        $dom[$key]['parent'] = end($level);
                        if (!self::isSelfClosingTag($element, $tagname)) {
        ...
        public function writeHTML($html, $ln = true, $fill = false, $reseth = false, $cell = false, $align = '')
        {
            ...
            if (isset($this->PageAnnots[$this->page])) {
                $pask = safeCount($this->PageAnnots[$this->page]);
            } else {
                $pask = 0;
            }
                            ...
                            if (isset($this->PageAnnots[$startlinepage])) {
                                foreach ($this->PageAnnots[$startlinepage] as $pak => $pac) {
                                    if ($pak >= $pask) {
                                        ...
                                        $npak = safeCount($this->PageAnnots[$this->page]) - 1;
                                        ...
                                        switch ($strpiece[2][0]) {
                                            ...
                                            case 'l':
                                            {
                                                ...
                                                $pmid = preg_replace_callback('/([0-9\.\+\-]*)[\s](' . $strpiece[1][0] . ')[\s](' . $strpiece[2][0] . ')([\s]*)/x',
                                                    function ($matches) {
                                                        global $spacew;
                                                        $newx = sprintf('%.2F', ((float)$matches[1] + $spacew));
                                            ...
                                            case 'c':
                                            {
                                                ...
                                                $pmid = preg_replace_callback('/([0-9\.\+\-]*)[\s]([0-9\.\+\-]*)[\s]([0-9\.\+\-]*)[\s]([0-9\.\+\-]*)[\s]([0-9\.\+\-]*)[\s](' . $strpiece[1][0] . ')[\s](' . $strpiece[2][0] . ')([\s]*)/x',
                                                    function ($matches) {
                                                        global $spacew;
                                                        $newx1 = sprintf('%.3F', ((float)$matches[1] + $spacew));
                                                        $newx2 = sprintf('%.3F', ((float)$matches[3] + $spacew));
                                                        $newx3 = sprintf('%.3F', ((float)$matches[5] + $spacew));
                    ...
                    if ($dom[$key]['opening']) {
                        ...
                        if (($dom[$key]['value'] == 'td') or ($dom[$key]['value'] == 'th')) {
                            ...
                            if (isset($dom[$table_el]['rowspans'])) {
                                $rsk = 0;
                                $rskmax = safeCount($dom[$table_el]['rowspans']);
                                while ($rsk < $rskmax) {
        ...
        /**
         * Checks if the prodided element is a self-closing tag
         * @param string $element
         * @param string $tagname
         * @return bool
         */
        public static function isSelfClosingTag(string $element, string $tagname): bool
        {
            $voidElements = [
                'area',
                'base',
            ...
            if (!str_ends_with($element, '/') && !in_array($tagname, $voidElements)) {
                return false;
            }
            return true;
        }
