...
gulp.task('upgrade-lib', function(done) {
    const rm = (pathArr = []) => {
    ...
    commander
        .option('--name <name>', 'Name of upgrading library')
        .parse(process.argv);
    ...
    try {
        const upgradeConfig = require('./include/javascript/upgrade.config');

        if (upgradeConfig && upgradeConfig[commander.name]) {
            const cnf = upgradeConfig[commander.name];

            rm(cnf.removeBefore);

            cnf.copyFiles.map((item) => {
                gulp.src(item.from)
