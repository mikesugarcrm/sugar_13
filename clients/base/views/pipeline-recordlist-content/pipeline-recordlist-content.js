...
({
    ...
    handleSideDrawerCloses: function() {
        ...
        $('.table .blue-border').removeClass('blue-border');
        $('.table .current-column').removeClass('current-column');
        let tableResponsive = $('.table').closest('.table-responsive');
        if (tableResponsive.length) {
            tableResponsive.css('width', 'auto');
        }

        // Restore the ability for the user to scroll horizontally
        $('.my-pipeline-content').css('overflow-x', 'auto');

        $('.table tr').animate({
            left: 0
        }, sideDrawer.closingDuration, null, function() {
    ...
    setCurrentColumn: function(column, row, animationDuration = 0) {
        let $table = $('.table');
        let $td = $table.find('td').eq(column);
        ...
        // Calculate the 'left' CSS change based on column and scroll positions
        let scrollOffset = $table.closest('.my-pipeline-content').scrollLeft();
        let scrollPx = (($td.position().left + 6) - scrollOffset);

        $table.find('tr').animate({
            left: `${(-scrollPx * 0.0625)}rem`
        }, animationDuration, function() {
            // While focused on the column, don't let the user scroll away horizontally
            $('.my-pipeline-content').css('overflow-x', 'hidden');
            if ($('.arrowholder').length) {
