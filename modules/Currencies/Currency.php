...
class Currency extends SugarBean
{
    ...
    public function retrieveIDBySymbol($symbol)
    {
        ...
        $stmt = $conn->executeQuery($query, [$symbol, 0]);
    ...
    public function retrieveIDByISO($iso)
    {
        ...
        $conn = $this->db->getConnection();
        $stmt = $conn->executeQuery($query, [$iso, 0]);
    ...
    public function retrieveIDByName($name)
    {
        ...
        $conn = $this->db->getConnection();
        $stmt = $conn->executeQuery($query, [$name, 0]);
    ...
    public function save($check_notify = false)
    {
        ...
        MetaDataManager::refreshSectionCache([MetaDataManager::MM_CURRENCIES]);
...
function unformat_number($string, $useBaseCurrency = false)
{
    ...
    $string = trim(str_replace([$seps[0], $seps[1], $currencySymbol], ['', '.', ''], (string)$string));
