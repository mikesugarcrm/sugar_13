...
class EmailUI
{
    ...
    public $folderStates = []; // array of folderPath names and their states (1/0)
    public $smarty;
    public $addressSeparators = [';', ','];
     ...
     */
    public function displayEmailFrame()
    {
        ...
        $this->smarty->assign('dateFormatExample', str_replace(['Y', 'm', 'd'], ['yyyy', 'mm', 'dd'], $cuDatePref['date']));
        ...
        $emailCheckInterval = ['options' => $app_list_strings['email_check_interval_dom'], 'selected' => $emailSettings['emailCheckInterval']];
        ...
        ///////////////////////////////////////////////////////////////////////
        ////	FOLDERS & TreeView
        $this->smarty->assign('groupUserOptions', $ie->getGroupsWithSelectOptions(['' => $app_strings['LBL_EMAIL_CREATE_NEW']]));
...
eoq;
    ...
    public function displayQuickComposeEmailFrame()
    {
        ...
        $outData = ['jsData' => $javascriptOut, 'divData' => $divOut];
    ...
    public function generateComposePackageForQuickCreate($composeData, $fullLinkUrl, $lazyLoad = false, $bean = null)
    {
        ...
        $quickComposeOptions = ['fullComposeUrl' => $fullLinkUrl, 'composePackage' => $composePackage];
    ...
    public function _generateComposeConfigData($type = 'email_compose_light')
    {
        ...
        $sigJson = !empty($defaultSignature) ? json_encode([$defaultSignature['id'] => from_html($defaultSignature['signature_html'])]) : 'new Object()';
    ...
    public function retrieveTeamInfoForSettingsUI()
    {
        global $current_user;
        $e2UserPreferences = $this->getUserPrefsJS();
        $emailSettings = $e2UserPreferences['emailSettings'];
    ...
    public function addContactsToList($list_id, $contacts, $newName, $truncate = false)
    {
        ...
        if (is_array($contacts)) {
            for ($i = 0; $i < safeCount($contacts); $i++) {
    ...
    public function getUserPrefsJS()
    {
        ...
        $focusFolder = !empty($focusFolder) ? unserialize($focusFolder, ['allowed_classes' => false]) : [];
    ...
    public function buildTreeNode($key, $label, $mbox, $ieId, $nodePath, $isGroup, $ie)
    {
        ...
        if (safeCount($exMbox) >= 2) {
            $mailbox = '';
            for ($i = 2; $i < safeCount($exMbox); $i++) {
                if ($mailbox != '') {
                    $mailbox .= '.';
                }
                $mailbox .= "{$exMbox[$i]}";
    ...
    public function getQuickCreateForm($vars, $email, $addToAddressBookButton = false)
    {
        ...
        $EditView->defs['templateMeta']['form']['buttons'] = [
            'email2save' => [
                'id' => 'e2AjaxSave',
        ...
        if ($addToAddressBookButton) {
            $EditView->defs['templateMeta']['form']['buttons']['email2saveAddToAddressBook'] = [
...
EOQ;
    ...
    public function getDetailViewForEmail2($emailId)
    {
        ...
        $title = '';
        if ($focus->type == 'out') {
            $title = getClassicModuleTitle('Emails', [$mod_strings['LBL_SENT_MODULE_NAME'], $focus->name], true);
        } elseif ($focus->type == 'draft') {
            $title = getClassicModuleTitle('Emails', [$mod_strings['LBL_LIST_FORM_DRAFTS_TITLE'], $focus->name], true);
        } elseif ($focus->type == 'inbound') {
            $title = getClassicModuleTitle('Emails', [$mod_strings['LBL_INBOUND_TITLE'], $focus->name], true);
        }
        ...
        $attachments = '';
        for ($i = 0; $i < safeCount($notes_list); $i++) {
        ...
        if (strpos($folder, 'sugar::') !== false) {
            ...
            switch ($type) {
                case 'deleted':
                    ...
                    $ieX->retrieve_by_string_fields(['groupfolder_id' => $ieId, 'deleted' => 0]);
            ...
            if ($type == 'deleted') {
                ...
                if (!empty($manipulated)) {
                    $ie->setCacheValue($ie->mailbox, [], $manipulated);
            ...
            foreach ($uids as $uid) {
                ...
                if (!empty($msgNo)) {
                    if ($ie->importOneEmail($msgNo, $uid)) {
                        ...
                        $out = $out . string_format($mod_strings['ERR_MSG_FAILED'], [$messageIndex]) . " \r\n";
                    }
                }
                $messageIndex++;
            } // for
    ...
    public function distDirect($user, $mailIds)
    {
        ...
        foreach ($mailIds as $k => $mailId) {
            ...
            if (!empty($email->teams) && $assignedTeamInfo) {
                ...
                $teamIds = explode(',', $_REQUEST['team_ids']);
                $email->teams->$updateType($teamIds, [], false);
...
eoq;
    ...
    public function displayComposeEmail($email)
    {
        ...
        $addresses = ['toAddresses' => 'to', 'ccAddresses' => 'cc', 'bccAddresses' => 'bcc'];
        foreach ($addresses as $var => $type) {
            ${$var} = '';
            foreach (["{$type}_addrs_names", "{$type}addrs", "{$type}_addrs"] as $emailVar) {
    ...
    public function _getPeopleUnionQuery($whereArr, $person)
    {
        ...
        return ['query' => $q, 'countQuery' => $countq];
    }
    ...
    public function getRelatedEmail($beanType, $whereArr, $relatedBeanInfoArr = '')
    {
        ...
        return ['query' => $finalQuery, 'countQuery' => $countq];
    }
    ...
    public function generateDynamicFolderQuery($type, $userId)
    {
        ...
        $replacee = ['::TYPE::', '::STATUS::', '::USER_ID::'];
        $replacer = [$type, $status, $userId];
    ...
    public function clearInboundAccountCache($ieId)
    {
        ...
        $files = findAllFiles($cacheRoot . '/messages/', []);
        foreach ($files as $file) {
            unlink($file);
        } // fn
        $files = findAllFiles($cacheRoot . '/attachments/', []);
        foreach ($files as $file) {
            unlink($file);
        } // for
    } // fn
    ...
    public function getEmailTemplatesArray()
    {
        ...
        if (ACLController::checkAccess('EmailTemplates', 'list', true) && ACLController::checkAccess('EmailTemplates', 'view', true)) {
            ...
            $etResult = $et->db->query($et->create_new_list_query('', "(type IS NULL OR type='' OR type='email')", [], [], ''));
            $email_templates_arr = ['' => $app_strings['LBL_NONE']];
    ...
    public function getFromAccountsArray($ie)
    {
        ...
        foreach ($ieAccountsFull as $k => $v) {
            ...
            if ($name != null && $addr != null) {
                $name = from_html($name);
                if (!$v->is_personal) {
                    $ieAccountsFrom[] = ['value' => $v->id, 'text' => "{$name} ({$addr}) - {$app_strings['LBL_EMAIL_UPPER_CASE_GROUP']}"];
                } else {
                    $ieAccountsFrom[] = ['value' => $v->id, 'text' => "{$name} ({$addr})"];
                } // else
            } // if
        } // foreach
        ...
        if (!empty($system->mail_smtpserver)) {
            ...
            $ieAccountsFrom[] = [
                'value' => $system->id,
                'text' => "{$ret['name']} ({$ret['email']}){$myAccountString}",
            ];
        }
    ...
    public function getFromAllAccountsArray($ie, $ret)
    {
        ...
        foreach ($ieAccountsFull as $k => $v) {
            ...
            if ($foundInPersonalAccounts) {
                if ($v->is_personal) {
                    $ieAccountsFrom[] = ['value' => $v->id, 'selected' => $selected, 'text' => "{$storedOptionsName} ({$storedOptions['from_addr']})"];
                } // if
            } else {
                $ieAccountsFrom[] = ['value' => $v->id, 'selected' => $selected, 'text' => "{$storedOptionsName} ({$storedOptions['from_addr']}) - {$app_strings['LBL_EMAIL_UPPER_CASE_GROUP']}"];
            } // else
        } // foreach
    ...
    public function getShowAccountsOptions(&$ie)
    {
        ...
        foreach ($ieAccountsFull as $k => $v) {
            ...
            $ieAccountsShowOptionsMeta[] = ['id' => $v->id, 'name' => $v->name, 'is_active' => $selected,
        ...
        foreach ($groupFolders as $singleGroup) {
            ...
            if (safeCount($relatedIEAccounts) == 0) {
                $server_url = $app_strings['LBL_EMAIL_MULT_GROUP_FOLDER_ACCOUNTS_EMPTY'];
            } elseif (safeCount($relatedIEAccounts) == 1) {
            ...
            $ieAccountsShowOptionsMeta[] = ['id' => $singleGroup['id'], 'name' => $singleGroup['origName'], 'is_active' => $singleGroup['selected'],
    ...
    public function getShowAccountsOptionsForSearch(&$ie)
    {
        ...
        $ieAccountsShowOptionsMeta = [];
        $ieAccountsShowOptionsMeta[] = ['value' => '', 'text' => $app_strings['LBL_NONE'], 'selected' => ''];
        $showFolders = unserialize(base64_decode($current_user->getPreference('showFolders', 'Emails')), ['allowed_classes' => false]);
        ...
        foreach ($ieAccountsFull as $k => $v) {
            ...
            $group = (!$v->is_personal) ? $app_strings['LBL_EMAIL_GROUP'] . '.' : '';
            $ieAccountsShowOptionsMeta[] = ['value' => $v->id, 'text' => $group . $v->name, 'protocol' => $v->protocol];
        }
    ...
    public function getCacheValue($ieId, $type, $file, $key)
    {
        ...
        if (file_exists($cacheFilePath)) {
            ...
            $this->writeCacheFile('retArr', [], $ieId, $type, $file);
        }
...
eoq;
    ...
    public function jsonOuput($data, $resultsParam, $count = 0, $fromCache = true, $unread = -1)
    {
        ...
        $jsonOut = ['TotalCount' => $count, 'FromCache' => $cached, 'UnreadCount' => $unread, $resultsParam => $data['out']];
