...
class Email extends SugarBean
{
    ...
    // fields which use gzip when saved and revert when fetched
    private const FIELDS_TO_GZIP = [
        'description',
        'description_html',
    ];
    ...
    public function email2Send($request)
    {
                ...
                foreach ($exAttachments as $file) {
                    ...
                    if (!empty($file)) {
                        ...
                        if ((($this->type == 'draft') && !empty($this->id)) || (isset($request['saveToSugar']) && $request['saveToSugar'] == 1)) {
                            ...
                            $noteteamIdsArray = (isset($_REQUEST['teamIds']) ? explode(',', $_REQUEST['teamIds']) : [$current_user->getPrivateTeamID()]);
        ...
        if (isset($_REQUEST['composeType']) && ($_REQUEST['composeType'] == 'reply' || $_REQUEST['composeType'] == 'replyCase')) {
            if (isset($_REQUEST['ieId']) && isset($_REQUEST['mbox'])) {
                $emailFromIe = BeanFactory::getBean('InboundEmail', $_REQUEST['ieId'], ['disable_row_level_security' => true]);
        ...
        if ($forceSave ||
            ...
            $teamIdsArray = (!empty($_REQUEST['teamIds']) ? explode(',', $_REQUEST['teamIds']) : [$current_user->getPrivateTeamID()]);
    ...
    public function getNamePlusEmailAddressesForCompose($module, $idsArray)
    {
        ...
        foreach ($idsArray as $id) {
            ...
            if (!empty($bean)) {
                ...
                elseif ($bean->ACLFieldAccess('email2', 'read')) {
    ...
    protected function updateAttachmentsVisibility()
    {
        if ($this->load_relationship('attachments')) {
            ...
            $GLOBALS['log']->debug(sprintf($message, safeCount($attachments), $this->id, __METHOD__));
     ...
     */
    public function saveTempNoteAttachments($filename, $fileLocation, $mimeType, $uploadId = null)
    {
     ...
     */
    public function loadAdditionalEmailData(SugarBean $emailBean = null)
    {
    ...
    public function retrieveEmailAddresses()
    {
        ...
        $conn = $this->db->getConnection();
        $stmt = $conn->executeQuery($query, [$this->id, 0]);
    ...
    public function retrieveEmailText()
    {
        ...
        $this->ungzipContentFields();
        $this->description_html = $ie->getHTMLDisplay(SugarCleaner::cleanHtml($this->description_html));
    }
    ...
    public function mark_deleted($id)
    {
        ...
        $this->db->updateParams(
            ...
            ['deleted' => 1],
            ['polymorphic_id' => $id, 'polymorphic_module' => 'Emails']
        );
    ...
    public function getSystemDefaultEmail()
    {
        ...
        $conn = $this->db->getConnection();
        $stmt = $conn->executeQuery($query, ['fromaddress']);
        $fromAddress = $stmt->fetchOne();
        $email['email'] = !empty($fromAddress) ? $fromAddress : '';

        $stmt = $conn->executeQuery($query, ['fromname']);
    ...
    public function create_new_list_query(
        ...
        $this->addVisibilityFrom($query, ['where_condition' => true]);
        ...
        $this->addVisibilityWhere($where_auto, ['where_condition' => true]);
...
SQL;
    ...
    public function searchImportedEmails($sort = '', $direction = '')
    {
        ...
        $direction = !empty($_REQUEST['dir']) && in_array(strtolower($_REQUEST['dir']), ['asc', 'desc']) ? $_REQUEST['dir'] : '';
    ...
    public function _genereateSearchImportedEmailsQuery()
    {
        ...
        } elseif (!empty($_REQUEST['attachmentsSearch']) && $_REQUEST['attachmentsSearch'] == 2) {
            $query['where'] .= ' AND NOT EXISTS ( SELECT id FROM notes n WHERE n.email_id = emails.id AND n.deleted = 0 AND n.filename is not null )';
        }

        $this->addVisibilityWhere($query['where'], ['where_condition' => true]);
    ...
    public function _generateSearchImportWhereClause()
    {
        ...
        $availableSearchParam = ['name' => ['table_name' => 'emails'],
        ...
        if (safeCount($additionalWhereClause)) {
            $GLOBALS['log']->debug('additionalWhereClause: ' .
                $additionalWhereClause[safeCount($additionalWhereClause) - 1]);
        }
        $GLOBALS['log']->debug('---------------------------------------------------------------------------------');
...
SQL;
    ...
    public function setFieldNullable($field)
    {
        ...
        if (isset($this->field_defs[$field]) && is_array($this->field_defs[$field])) {
            if (empty($this->modifiedFieldDefs[$field])) {
                if (isset($this->field_defs[$field]['isnull']) &&
    ...
    public function supportsGzip(): bool
    {
        ...
        $isSupported = $isSupported ?? function_exists('gzdeflate');
        return $isSupported;
    }

    public function gzipContentFields($emailTextBean = null): void
    {
        if (!$this->supportsGzip()) {
            return;
        }
        $emailTextBean = $emailTextBean ?? $this;
        foreach (self::FIELDS_TO_GZIP as $fieldName) {
            $value = $emailTextBean->$fieldName;
            if (!empty($value) && is_string($value)) {
                $value = $this->db->decodeHTML($value);
                $emailTextBean->$fieldName = $this->gzipContent($value);
            }
        }
    }

    public function ungzipContentFields($emailTextBean = null): void
    {
        if (!$this->supportsGzip()) {
            return;
        }
        $emailTextBean = $emailTextBean ?? $this;
        foreach (self::FIELDS_TO_GZIP as $fieldName) {
            if (!empty($emailTextBean->$fieldName) && is_string($emailTextBean->$fieldName)) {
                $emailTextBean->$fieldName = $this->ungzipContent($emailTextBean->$fieldName);
            }
        }
    }

    public function ungzipContent(string $content): string
    {
        return $this->tryUngzipContent($content) ?? $content;
    }

    public function tryUngzipContent(string $content): ?string
    {
        $base64Decoded = base64_decode($content, true);
        if ($base64Decoded === false) {
            return null;
        }
        $gzResult = @gzinflate($base64Decoded);

        return $gzResult !== false ? $gzResult : null;
    }

    public function gzipContent(string $content): string
    {
        return base64_encode((string)gzdeflate($content, 7));
    }
