...
class ImportViewConfirm extends ImportView
{
    ...
    public function display()
    {
        ...
        $importSource = $this->request->getValidInputRequest('source', ['Assert\Choice' => ['choices' => self::getImportSourceOptions()]], 'csv');
        ...
        $customEnclosure = $this->request->getValidInputRequest(
            ...
            [
                ...
                'Assert\Length' => ['min' => 0, 'max' => 1, 'allowEmptyString' => true],
            ],
            ''
        );
        ...
        if (!empty($maxLineCount) && ($lineCount > $maxLineCount)) {
            ...
            $maxRecordsWarningMessg = string_format($mod_strings['LBL_IMPORT_ERROR_MAX_REC_LIMIT_REACHED'], [$lineCount, $maxLineCount]);
        }
    ...
    private function getImportMap($importSource)
    {
        ...
        if (strncasecmp('custom:', $importSource, 7) == 0) {
            ...
            $import_map_seed = BeanFactory::getBean('Import_1', $id, ['encode' => false]);
    ...
    private function setNameFormatProperties($field_map = [])
    {
    ...
    private function setNumberFormatOptions($field_map = [])
    {
    ...
    private function setCurrencyOptions($field_map = [])
    {
        global $locale, $current_user;
...
eoq;
    ...
    private function setDateTimeProperties($field_map = [])
    {
    ...
    private function setImportFileCharacterSet($importFile, $field_map = [])
    {
        global $locale;
    ...
    {
        $results = [];
        $importMappings = ['ImportMapSalesforce', 'ImportMapOutlook'];
        foreach ($importMappings as $importMap) {
            $tmpFile = "modules/Import/maps/$importMap.php";
            if (file_exists($tmpFile)) {
                require_once $tmpFile;
                $t = new $importMap();
                $results[$t->name] = ['delim' => $t->delimiter, 'enclos' => $t->enclosure, 'has_header' => $t->has_header];
            }
        }
        return $results;
    }
    ...
    public function getMaxColumnsInSampleSet($sampleSet)
    {
        ...
        foreach ($sampleSet as $v) {
            if (safeCount($v) > $maxColumns) {
                $maxColumns = safeCount($v);
            }
        }
...
EOJAVASCRIPT;
    ...
    protected function _showImportError($message, $module, $action = 'Step1', $showCancel = false, $cancelLabel = null, $display = false)
    {
        ...
        $ss->assign('SOURCE', $this->request->getValidInputRequest('source', ['Assert\Choice' => ['choices' => self::getImportSourceOptions()]]));
