...
// build table datastructure in javascript objs
    ...
    foreach ($report_modules as $module_name => $bean_name) {
        ...
        foreach ($linked_fields as $linked_field) {
            ...
            echo "link_defs_{$module_name_escaped}[ '{$linked_field['name']}' ] = " . json_encode([
        ...
        if (is_array($module->field_defs)) {
            $module->ACLFilterFieldList($module->field_defs, ['owner_override' => true]);
            ...
            foreach ($module->field_defs as $field_def) {
                ...
                $allowed_fields_array = ['full_name'];
...
// END HALF-FIX
        ...
        ?>
        var default_table_columns_<?php echo $module_name_escaped; ?> = ["<?php echo implode('","', []); ?>"];
    ...
    qualifiers[qualifiers.length] = {name:'equals',value:<?php echo json_encode($mod_strings['LBL_EQUALS']); ?>};
    qualifiers[qualifiers.length] = {name:'not_equals_str',value:<?php echo json_encode($mod_strings['LBL_DOES_NOT_EQUAL']); ?>};
    qualifiers[qualifiers.length] = {name:'empty',value:<?php echo json_encode($mod_strings['LBL_IS_EMPTY']); ?>};
    qualifiers[qualifiers.length] = {name:'not_empty',value:<?php echo json_encode($mod_strings['LBL_IS_NOT_EMPTY']); ?>};
    qualifiers[qualifiers.length] = {name:'anything',value:<?php echo json_encode($mod_strings['LBL_ANYTHING']); ?>};
    filter_defs['encrypt'] = qualifiers;

    var qualifiers =  new Array();
    qualifiers[qualifiers.length] = {name:'equals',value:<?php echo json_encode($mod_strings['LBL_EQUALS']); ?>};
    qualifiers[qualifiers.length] = {name:'not_equals_str',value:<?php echo json_encode($mod_strings['LBL_DOES_NOT_EQUAL']); ?>};
    qualifiers[qualifiers.length] = {name:'contains',value:<?php echo json_encode($mod_strings['LBL_CONTAINS']); ?>};
    qualifiers[qualifiers.length] = {name:'does_not_contain',value:<?php echo json_encode($mod_strings['LBL_DOES_NOT_CONTAIN']); ?>};
    qualifiers[qualifiers.length] = {name:'starts_with',value:<?php echo json_encode($mod_strings['LBL_STARTS_WITH']); ?>};
    qualifiers[qualifiers.length] = {name:'ends_with',value:<?php echo json_encode($mod_strings['LBL_ENDS_WITH']); ?>};
    qualifiers[qualifiers.length] = {name:'empty',value:<?php echo json_encode($mod_strings['LBL_IS_EMPTY']); ?>};
    qualifiers[qualifiers.length] = {name:'not_empty',value:<?php echo json_encode($mod_strings['LBL_IS_NOT_EMPTY']); ?>};
    qualifiers[qualifiers.length] = {name:'anything',value:<?php echo json_encode($mod_strings['LBL_ANYTHING']); ?>};
    ...
    var is_def = {name:'is',value:<?php echo json_encode($mod_strings['LBL_IS']); ?>};
    var is_not_def = {name:'is_not',value:<?php echo json_encode($mod_strings['LBL_IS_NOT']); ?>};
    var one_of_def = {name:'one_of',value:<?php echo json_encode($mod_strings['LBL_ONE_OF']); ?>};
    var not_one_of_def = {name:'not_one_of',value:<?php echo json_encode($mod_strings['LBL_IS_NOT_ONE_OF']); ?>};
    ...
    var qualifiers_name = new Array();
    var is_not_empty_def = {name:'not_empty',value:<?php echo json_encode($mod_strings['LBL_IS_NOT_EMPTY']); ?>};
    var is_empty_def = {name:'empty',value:<?php echo json_encode($mod_strings['LBL_IS_EMPTY']); ?>};
    var reports_to_def = {name:'reports_to',value:<?php echo json_encode($mod_strings['LBL_REPORTS_TO']); ?>};
    var anything = {name:'anything',value:<?php echo json_encode($mod_strings['LBL_ANYTHING']); ?>};
    qualifiers_name.unshift(anything);
    ...
    var qualifiers =  new Array();
    qualifiers[qualifiers.length] = {name:'on',value:<?php echo json_encode($mod_strings['LBL_ON']); ?>};
    qualifiers[qualifiers.length] = {name:'before',value:<?php echo json_encode($mod_strings['LBL_BEFORE']); ?>};
    qualifiers[qualifiers.length] = {name:'after',value:<?php echo json_encode($mod_strings['LBL_AFTER']); ?>};
    qualifiers[qualifiers.length] = {name:'between_dates',value:<?php echo json_encode($mod_strings['LBL_IS_BETWEEN']); ?>};
    qualifiers[qualifiers.length] = {name:'not_equals_str',value:<?php echo json_encode($mod_strings['LBL_NOT_ON']); ?>};
    qualifiers[qualifiers.length] = {name:'empty',value:<?php echo json_encode($mod_strings['LBL_IS_EMPTY']); ?>};
    qualifiers[qualifiers.length] = {name:'not_empty',value:<?php echo json_encode($mod_strings['LBL_IS_NOT_EMPTY']); ?>};


    qualifiers[qualifiers.length] = {name:'tp_yesterday',value:<?php echo json_encode($mod_strings['LBL_YESTERDAY']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_today',value:<?php echo json_encode($mod_strings['LBL_TODAY']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_tomorrow',value:<?php echo json_encode($mod_strings['LBL_TOMORROW']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_last_n_days',value:<?php echo json_encode($mod_strings['LBL_LAST_N_DAYS']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_next_n_days',value:<?php echo json_encode($mod_strings['LBL_NEXT_N_DAYS']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_last_7_days',value:<?php echo json_encode($mod_strings['LBL_LAST_7_DAYS']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_next_7_days',value:<?php echo json_encode($mod_strings['LBL_NEXT_7_DAYS']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_last_month',value:<?php echo json_encode($mod_strings['LBL_LAST_MONTH']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_this_month',value:<?php echo json_encode($mod_strings['LBL_THIS_MONTH']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_next_month',value:<?php echo json_encode($mod_strings['LBL_NEXT_MONTH']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_last_30_days',value:<?php echo json_encode($mod_strings['LBL_LAST_30_DAYS']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_next_30_days',value:<?php echo json_encode($mod_strings['LBL_NEXT_30_DAYS']); ?>};

    qualifiers[qualifiers.length] = {name:'tp_last_quarter',value:<?php echo json_encode($mod_strings['LBL_LAST_QUARTER']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_this_quarter',value:<?php echo json_encode($mod_strings['LBL_THIS_QUARTER']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_next_quarter',value:<?php echo json_encode($mod_strings['LBL_NEXT_QUARTER']); ?>};

    qualifiers[qualifiers.length] = {name:'tp_last_year',value:<?php echo json_encode($mod_strings['LBL_LAST_YEAR']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_this_year',value:<?php echo json_encode($mod_strings['LBL_THIS_YEAR']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_next_year',value:<?php echo json_encode($mod_strings['LBL_NEXT_YEAR']); ?>};
    ...
    qualifiers[qualifiers.length] = {name:'before',value:<?php echo json_encode($mod_strings['LBL_BEFORE']); ?>};
    qualifiers[qualifiers.length] = {name:'after',value:<?php echo json_encode($mod_strings['LBL_AFTER']); ?>};
    qualifiers[qualifiers.length] = {name:'between_datetimes',value:<?php echo json_encode($mod_strings['LBL_IS_BETWEEN']); ?>};
    qualifiers[qualifiers.length] = {name:'not_equals_str',value:<?php echo json_encode($mod_strings['LBL_NOT_ON']); ?>};
    qualifiers[qualifiers.length] = {name:'empty',value:<?php echo json_encode($mod_strings['LBL_IS_EMPTY']); ?>};
    qualifiers[qualifiers.length] = {name:'not_empty',value:<?php echo json_encode($mod_strings['LBL_IS_NOT_EMPTY']); ?>};
    qualifiers[qualifiers.length] = {name:'tp_yesterday',value:<?php echo json_encode($mod_strings['LBL_YESTERDAY']); ?>};
    ...
    var qualifiers =  new Array();
    qualifiers[qualifiers.length] = {name:'equals',value:<?php echo json_encode($mod_strings['LBL_EQUALS']); ?>};
    qualifiers[qualifiers.length] = {name:'not_equals',value:<?php echo json_encode($mod_strings['LBL_DOES_NOT_EQUAL']); ?>};
    qualifiers[qualifiers.length] = {name:'less',value:<?php echo json_encode($mod_strings['LBL_LESS_THAN']); ?>};
    qualifiers[qualifiers.length] = {name:'less_equal',value:<?php echo json_encode($mod_strings['LBL_LESS_THAN_EQUAL']); ?>};
    qualifiers[qualifiers.length] = {name:'greater_equal',value:<?php echo json_encode($mod_strings['LBL_GREATER_THAN_EQUAL']); ?>};
    qualifiers[qualifiers.length] = {name:'greater',value:<?php echo json_encode($mod_strings['LBL_GREATER_THAN']); ?>};
    qualifiers[qualifiers.length] = {name:'between',value:<?php echo json_encode($mod_strings['LBL_IS_BETWEEN']); ?>};
    qualifiers[qualifiers.length] = {name:'empty',value:<?php echo json_encode($mod_strings['LBL_IS_EMPTY']); ?>};
    qualifiers[qualifiers.length] = {name:'not_empty',value:<?php echo json_encode($mod_strings['LBL_IS_NOT_EMPTY']); ?>};
    qualifiers[qualifiers.length] = {name:'anything',value:<?php echo json_encode($mod_strings['LBL_ANYTHING']); ?>};
    ...
    var qualifiers =  new Array();
    qualifiers[qualifiers.length] = {name:'is',value:<?php echo json_encode($mod_strings['LBL_IS']); ?>};
    qualifiers[qualifiers.length] = {name:'is_not',value:<?php echo json_encode($mod_strings['LBL_IS_NOT']); ?>};
    qualifiers[qualifiers.length] = {name:'one_of',value:<?php echo json_encode($mod_strings['LBL_ONE_OF']); ?>};
    qualifiers[qualifiers.length] = {name:'not_one_of',value:<?php echo json_encode($mod_strings['LBL_IS_NOT_ONE_OF']); ?>};
    qualifiers[qualifiers.length] = {name:'empty',value:<?php echo json_encode($mod_strings['LBL_IS_EMPTY']); ?>};
    qualifiers[qualifiers.length] = {name:'not_empty',value:<?php echo json_encode($mod_strings['LBL_IS_NOT_EMPTY']); ?>};
    qualifiers[qualifiers.length] = {name:'anything',value:<?php echo json_encode($mod_strings['LBL_ANYTHING']); ?>};
    ...
    date_group_defs[date_group_defs.length] = {name:'day', value:<?php echo json_encode($mod_strings['LBL_BY_DAY']); ?>};
    date_group_defs[date_group_defs.length] = {name:'week', value:<?php echo json_encode($mod_strings['LBL_BY_WEEK']); ?>};
    date_group_defs[date_group_defs.length] = {name:'month', value:<?php echo json_encode($mod_strings['LBL_BY_MONTH']); ?>};
    date_group_defs[date_group_defs.length] = {name:'year', value:<?php echo json_encode($mod_strings['LBL_BY_YEAR']); ?>};
    date_group_defs[date_group_defs.length] = {name:'quarter', value:<?php echo json_encode($mod_strings['LBL_BY_QUARTER']); ?>};

    var qualifiers = new Array();
    qualifiers[qualifiers.length] = {name:'any',value:<?php echo json_encode($mod_strings['LBL_ANY']); ?>};
    qualifiers[qualifiers.length] = {name:'all',value:<?php echo json_encode($mod_strings['LBL_ALL']); ?>};
    qualifiers[qualifiers.length] = {name:'exact',value:<?php echo json_encode($mod_strings['LBL_EXACT']); ?>};
    ...
    filter_defs['file'] = [
    {name:'empty',value:<?php echo json_encode($mod_strings['LBL_IS_EMPTY']); ?>},
    {name:'not_empty',value:<?php echo json_encode($mod_strings['LBL_IS_NOT_EMPTY']); ?>}
    ];
