...
class InboundEmail extends SugarBean
{
    ...
    // default attributes
    public $transferEncoding = [0 => '7BIT',
        1 => '8BIT',
        2 => 'BINARY',
        3 => 'BASE64',
        4 => 'QUOTED-PRINTABLE',
    ...
    public $required_fields = ['name' => 'name',
    ...
    public $inlineImages = [];  // temporary space to store ID of inlined images
    ...
    public function deleteCache()
    {
        ...
        $conn->executeStatement($q, [$this->id]);
    }
     ...
     * Sets cache values
     */
    public function setCacheValue($mbox, array $insert, array $update = [], array $remove = [])
    {
        if (empty($mbox)) {
            return;
        }
    ...
    public function pop3_checkPartialEmail($synch = false)
    {
        ...
        if (file_exists($cacheFilePath)) {
            ...
            if ($fh = @fopen($cacheFilePath, 'rb')) {
                ...
                if (!empty($diff)) {
                    if (safeCount($diff) > 50) {
                        $newDiff = array_slice($diff, 50, safeCount($diff), true);
                    } else {
                        $newDiff = [];
                    }
        ...
        if (safeCount($diff) > 0) {
            ...
            return [
                'status' => 'In Progress',
                'mbox' => $this->mailbox,
                'count' => (safeCount($results) + $startingNo),
                'totalcount' => safeCount($diff),
                'ieid' => $this->id,
            ];
        } // if
    ...
    public function checkEmailOneMailbox($mailbox, $prefetch = true, $synchronize = false)
    {
        ...
        $GLOBALS['log']->info(
            "[EMAIL] Done IMAP search on mailbox [{$mailbox}] for user [{$current_user->user_name}]. Result count = " . safeCount($searchResults)
        );
    ...
    public function checkEmailOneMailboxPartial($mailbox, $prefetch = true, $synchronize = false, $start = 0, $max = -1)
    {
        ...
        if (!empty($searchResults)) {
            ...
            $ret = ['status' => $status, 'count' => $count, 'mbox' => $mailbox, 'totalcount' => $total];
    ...
    public function getMailboxProcessCount($mailbox)
    {
        ...
        return safeCount($searchResults);
    }
     ...
     * @return array
     */
    public function processRules($searchResults, $user = null)
    {
        global $current_user;
    ...
    public function search($ieId, $subject = '', $from = '', $to = '', $body = '', $dateFrom = '', $dateTo = '')
    {
        ...
        foreach ($beans as $bean) {
            ...
            foreach ($mailboxes as $mbox) {
                ...
                $numHits = safeCount($searchOverviews);
    ...
    public function findOptimumSettings(
        ...
        $nonSsl = ['both-secure' => '/notls/novalidate-cert/secure',
        ...
        $acceptableWarnings = ['SECURITY PROBLEM: insecure server advertised AUTH=PLAIN', //c-client auth_pla.c
            'Mailbox is empty'];
     ...
     */
    public function getGroupsWithSelectOptions($groups = [])
    {
        ...
        if (!$this->handleCaseAssignment($email) && $this->isMailBoxTypeCreateCase()) {
            ...
            if ($contactIds = $this->getRelatedId($contactAddr, 'contacts')) {
                if (!empty($contactIds) && $c->load_relationship('contacts')) {
                    if (!$accountIds && safeCount($contactIds) == 1) {
    ...
    public function addBreadCrumbOffset($bc, $offset)
    {
        ...
        if (safeCount($a_bc) < safeCount($a_offset)) {
            $a_bc = array_merge($a_bc, array_fill(safeCount($a_bc), safeCount($a_offset) - safeCount($a_bc), 0));
        }

        $results = [];
        for ($i = 0; $i < safeCount($a_bc); $i++) {
            if (isset($a_offset[$i])) {
     ...
     */
    public function buildBreadCrumbsHTML($parts, $breadcrumb = '0', $stackedBreadcrumbs = [])
    {
        $subtype = 'HTML';
        $disposition = 'inline';
        ...
        if ($forDisplay || $dupeCheckResult) {
            ...
            if (!empty($_REQUEST['team_ids'])) {
                ...
                if (!empty($email->teams)) {
                    ...
                    $email->teams->replace($teamIds, [], false);
...
eoq;
    ...
    public function getNewMessageIds()
    {
        ...
        $GLOBALS['log']->debug(
            '-----> getNewMessageIds() got ' . safeCount($ret) . ' new Messages'
        );
        return $ret;
    }
    ...
    public function getNewIds()
    {
        ...
        $GLOBALS['log']->debug('-----> getNewIds() got ' . safeCount($ids) . ' new Messages');
        return $ids;
    }
        ...
        if ($fromIe == $toIe) {
            ...
            if ($copy) {
                ...
                if ($imapStatus && imap_mail_move($this->conn, $uids, $toFolder, CP_UID)) {
                    ...
                    $this->setCacheValue($toFolder, $overviews, [], []);
    ...
    public function getNewEmailsForSyncedMailbox()
    {
        ...
        try {
            if (safeCount($msgIds) > 0) {
                ...
                arsort($msgIds);
                $GLOBALS['log']->debug('-----> getNewEmailsForSyncedMailbox() got ' . safeCount($msgIds) . ' Messages');
        ...
        $GLOBALS['log']->debug('-----> getNewEmailsForSyncedMailbox() got ' . safeCount($result) . ' unsynced messages');
        return $result;
    }
