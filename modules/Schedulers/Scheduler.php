...
class Scheduler extends SugarBean
{
    ...
    /**
     * check if the Job has license to run
     * @return bool
     */
    protected function hasLicenseToRun(string $name): bool
    {
        return AccessControlManager::instance()->allowJobAccess($name);
    }
    ...
    public function checkPendingJobs($queue)
    {
        ...
        $GLOBALS['log']->info(
            '-----> Scheduler found [ ' . safeCount($allSchedulers) . ' ] ACTIVE jobs'
        );

        if (!empty($allSchedulers)) {
            foreach ($allSchedulers as $focus) {
                if (!empty($focus->name) && $this->hasLicenseToRun($focus->name) && $focus->fireQualified()) {
                    $job = $focus->createJob();
                    $queue->submitJob($job);
                } elseif (!empty($focus->name) && !$this->hasLicenseToRun($focus->name)) {
                    $GLOBALS['log']->info('Job: ' . $focus->name . ' has no license to run');
                }
            }
        } else {
