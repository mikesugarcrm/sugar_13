...
class DynamicField
{
    ...
    public static $fieldTypeRangeValue = [
        'int32' => [
            self::TYPE_SIGNED => [
                'max' => 2147483647,
                'min' => -2147483648,
    ...
    public function buildCache($module = false, $saveCache = true)
    {
        ...
        if (!empty($module)) {
            ...
            $results = []; // clear out results - if we remove a module we don't want to have its old vardefs hanging around
        }
    ...
    public function getFieldWidget($module, $fieldName)
    {
        ...
        $stmt = $db->getConnection()->executeQuery($query, [$module, $fieldName]);
        require_once 'modules/DynamicFields/FieldCases.php';
    ...
    public function save($isUpdate)
    {
        ...
        if ($this->bean->hasCustomFields() && isset($this->bean->id)) {
            $fields = ['id_c' => ['name' => 'id_c', 'type' => 'id']];
            $values = ['id_c' => $this->bean->id];
            foreach ($this->bean->field_defs as $name => $field) {
                if (empty($field['source']) || $field['source'] != 'custom_fields') {
                    continue;
                }
                ...
                $fields[$name] = array_merge($field, ['source' => 'db']);
            ...
            if ($isUpdate) {
                $db->updateParams($this->bean->table_name . '_cstm', $fields, $values, [
                    'id_c' => $values['id_c'],
                ]);
            } else {
    ...
    public function deleteField($widget, ?bool $cleanup = true)
    {
        ...
        if ($cleanup) {
            ...
            MetaDataManager::refreshModulesCache([$this->module]);
        }
    }
    ...
    public function writeVardefExtension($bean_name, $field, $def_override)
    {
        ...
        foreach ($def_override as $property => $val) {
            $out .= override_value_to_string_recursive([$vBean, 'fields', $field->name, $property], 'dictionary', $val) . "\n";
        }
     ...
     * @return boolean
     */
    public function addField($name, $label = '', $type = 'Text', $max_size = '255', $required_option = 'optional', $default_value = '', $ext1 = '', $ext2 = '', $ext3 = '', $audited = 0, $mass_update = 0, $ext4 = '', $help = '', $duplicate_merge = 0, $comment = '')
    {
        require_once 'modules/DynamicFields/FieldCases.php';
    ...
    public function addLabel($displayLabel)
    {
        ...
        $parser->handleSave(['label_' . $systemLabel => $displayLabel], $GLOBALS ['current_language']);
    ...
    public function getDBName($name, $_C = true)
    {
        ...
        $exclusions = ['parent_type', 'parent_id', 'currency_id', 'parent_name', 'base_rate'];
    ...
    public function getAllFieldsView($view, $type)
    {
        ...
        foreach ($this->bean->field_defs as $name => $data) {
            ...
            switch (strtolower($type)) {
                case 'xtpl':
                    $results[$name] = ['xtpl' => $field->get_xtpl()];
                    break;
                case 'html':
                    $results[$name] = ['html' => $field->get_html(), 'label' => $field->get_html_label(), 'fieldType' => $field->data_type, 'isCustom' => true];
                    break;
            }
        }
        return $results;
